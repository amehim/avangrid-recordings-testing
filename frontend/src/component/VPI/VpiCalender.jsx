...
const VpiDateTimeInput = () => {
  ...
  const Calendar = ({ onSelect, onTimeChange, initialTime, fieldName, selectedDate }) => {
    // Initialize with current date or selected date
     ...
     // Initialize with 2014 as default year if no date is selected
  const parseDate = (dateStr) => {
    if (!dateStr) return new Date(2014, 0, 1); // Default to January 1, 2014
    try {
      const [year, month, day] = dateStr.split('-').map(Number);
      // Ensure year is within 2014-2022 range
      const validYear = Math.max(2014, Math.min(2022, year));
      return new Date(validYear, month - 1, day || 1);
    } catch (e) {
      console.error("Error parsing date:", e);
      return new Date(2014, 0, 1); // Default to January 1, 2014
    }
  };
    ...
    const yearsToDisplay = Array.from({ length: 9 }, (_, i) => startYear + i);
        ...
        <div className="flex justify-between items-center mb-2">
          ...
          <div className="flex gap-1">
            ...
            <select 
              ...
              className="text-xs border rounded py-0.5 px-1 w-16"
            >
  ...
  return (
    <div className="p-4 relative max-w-full">
      <div className="flex gap-2 items-end">
            ...
            <div className="flex-1 relative" key={field}>
              ...
              <div className="flex items-center w-48">
                <input
                  ...
                  className={`px-2 py-2.5 text-sm border rounded-l focus:outline-none focus:ring-1 focus:ring-blue-400 w-full ${
        ...
        <div className="mb-0 self-start"> 
          <label className="block text-sm font-medium text-gray-700 mb-1">OPCO:</label>
          <select
            value={selectedCompany}
            onChange={(e) => setSelectedCompany(e.target.value)}
            className="text-sm border border-gray-300 rounded py-2.5 px-3 bg-white shadow-sm focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 w-40"
          >
        ...
        <button
          ...
          className="bg-blue-500 hover:bg-blue-600 text-white h-10 px-5 rounded items-center text-sm"
        >
          Search
        </button>
      </div>
...
export default VpiDateTimeInput;