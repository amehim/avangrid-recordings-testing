...
const AudioPlayer = ({ selectedTableRow }) => {
  ...
  // Get interaction ID from the selected row
  const interactionId = selectedTableRow?.Interaction_ID;
  ...
  // Load audio data when interaction ID changes or retry is triggered
  useEffect(() => {
    // Reset states
    setIsPlaying(false);
    setDuration(0);
    ...
    setIsLoading(true);
    console.log("Loading audio for interaction:", interactionId);

    // Fetch the audio file using axios
    const fetchAudio = async () => {
      try {
        const response = await api.get(`/talkdesk/recording?interactionId=${interactionId}`, {
          responseType: 'blob',
        });
  ...
  }, [interactionId, retryCounter]); // Added retryCounter dependency
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      // Use interaction ID for the filename
      const filename = `recording-${interactionId}.mp3`;
    ...
    return (
      <div className="bg-white rounded-md shadow-md p-4 h-full flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 font-medium">Loading recording for ID: {interactionId}...</p>
        </div>
      </div>
    );
  }
